...
def dataset_preparation(num_tasks=10):
    datasets, time_points = load_datasets('../data/CTD/MNIST50/dataset.pkl')
    X = np.stack([np.stack([np.asarray(img) for img in dataset[0]], axis=0) for dataset in datasets], axis=0)  # (D, N, 28, 28, 1)
    Y = np.stack([np.array(dataset[1]) for dataset in datasets], axis=0)  # (D, N)
    # X = X / 255
    X = np.transpose(X, (0, 1, 4, 2, 3))  # (D, N, 1, 28, 28)
    ...
    for i in range(train_tasks):
        temp_X = X[i]
        temp_Y = Y[i]
        domain_dataset = TensorDataset(torch.Tensor(temp_X).to(device), torch.Tensor(temp_Y).long().to(device))
...
def manual_forward(input, weights, biases):
    x = F.log_softmax(F.linear(
        F.relu(F.linear(
            input,
            weights['0.weight'], biases['0.bias'])),
        weights['3.weight'], biases['3.bias']), dim=1)
    return x
    ...
    for epoch in range(epoch_num):
        ...
        # for batch in np.array_split(idx_train, 10):
            ...
            for s in batch:
                ...
                Y = Y.view(-1)
                loss1 = loss1 + F.nll_loss(init_pred, Y)
                loss2 = loss2 + F.nll_loss(pred, Y)
                ...
                init_prediction = init_pred.argmax(dim=1)
                prediction = pred.argmax(dim=1)

                accuracy_init = accuracy_score(init_prediction.cpu().numpy(), Y.cpu().numpy())
                accuracy_gene = accuracy_score(prediction.cpu().numpy(), Y.cpu().numpy())
                acc1.append(accuracy_init), acc2.append(accuracy_gene)
        ...
        with torch.no_grad():
            ...
            for v in idx_val:
                ...
                Y = Y.view(-1)
                val_loss = val_loss + F.nll_loss(pred, Y)
...
def testModel(loaders, time_point, model, repeat):
    ...
    accuracy = accuracy_score(prediction.cpu().numpy(), Y.view(-1).cpu().numpy())
    ...
    n, c = test_pred.shape
    test_pred = test_pred.view((test_tasks, -1, c))
    for i in range(len(loaders)):
        test_pred_step, Y_step = test_pred[i], Y[i]
        prediction = test_pred_step.argmax(dim=1)
        accuracy = accuracy_score(prediction.cpu().numpy(), Y_step.cpu().numpy())
